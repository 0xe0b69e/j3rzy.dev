      ...
      {(!!file && (!file.isPrivate || session?.user.id === file.userId))
        ? (
          <div
            ...
            <h1
              className="text-3xl font-mono bg-background dark:bg-background-dark shadow-inner border-4 border-background dark:border-background-dark py-2 px-6"
              style={{ boxShadow: "inset 0 4px 8px rgba(0, 0, 0, 0.6)" }}
            >{file.name}</h1>
            <p>Uploaded by {uploader?.name ?? "Anonymous"}</p>
            <p>Size: {formatBytes(file.size)}</p>
            <p className="mb-3">Uploaded at: {formatDate(file.createdAt)}</p>
            <Link
        ...
        : (
          <div className="p-8 bg-background-light dark:bg-background-dark rounded-lg shadow-lg flex items-center justify-center text-center flex-col">
